...
def comp_vqc(x,y):
    ...
    if int(d1.split('_')[-1]) != int(d2.split('_')[-1]):
        return int(d1.split('_')[-1]) - int(d2.split('_')[-1])
...
if __name__ == "__main__":
    ...
    exp_dirs = [cfg["exp_dir"]]
    exp_names = [cfg["exp_name"]]
    ...
    for i, dir in enumerate(exp_dirs):
        final_acc = []
        sub_dir = sorted([d for d in dir.iterdir() if d.is_dir()], key=cmp_to_key(comp_vqc))
        cnt = 0
        col = 0
        for d in sub_dir:
            # Plot accuracy
